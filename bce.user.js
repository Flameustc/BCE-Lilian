...
const FBC_VERSION = "4.25-Lilian-202303271413400";
...
async function ForBetterClub() {
	...
	const defaultSettings = {
		...
		autoStruggle: {
			...
			sideEffects: (newValue) => {
				...
				if (newValue) {
					fbcSettings.autoStruggle = false;
				}
			},
			category: "cheats",
			description:
		...
		allowIMBypassBCX: {
			...
			sideEffects: (newValue) => {
				...
				if (newValue) {
					fbcSettings.allowIMBypassBCX = false;
				}
			},
			category: "cheats",
			description:
		...
		bcxLilian: {
			...
			sideEffects: (newValue) => {
				...
				debug("bcxLilian", newValue);
			},
			category: "addons",
			description:
			"Load the Lilian version of BCX.",
		},
		ebch: {
		...
		antiAntiGarbleExtra: {
			...
			category: "immersion",
			description:
				"Use Lilian's customized gag effect to prevent others from understanding you even those that are cheating.",
		},
		antiLoosenOwnerLock: {
			label:
			...
			value: true,
			sideEffects: (newValue) => {
				debug("antiLoosenOwnerLock", newValue);
				if (!newValue) {
					fbcSettings.antiLoosenOwnerLock = true;
				}		
			},
			category: "immersion",
			description:
		...
		itemAntiCheat: {
			...
			sideEffects: (newValue) => {
				...
				if (newValue) {
					fbcSettings.itemAntiCheat = false;
				}
			},
			category: "immersion",
			description:
		...
		showQuickAntiGarble: {
			...
			sideEffects: (newValue) => {
				...
				if (newValue) {
					fbcSettings.showQuickAntiGarble = false;
				}
			},
			category: "misc",
			description:
	...
	registerFunction(antiLoosen, "antiLoosen");
				...
				if (isCharacter(C) && canAccessLayeringMenus()) {
					...
					if (assetWorn(C, focusItem)) {
						if (fbcSettings.modifyDifficulty && canAccessDifficultyMenu()
								&& !(fbcSettings.antiLoosenOwnerLock && (focusItem.Asset.OwnerOnly || (lock && lock.Asset.OwnerOnly)) && !C.IsOwnedByPlayer())) {
							DrawButton(
								10,
								890,
								52,
	...
	async function antiGarbling() {
			...
			(args, next) => {
				...
				if (Sensitivity >= 2) {
					if (BCX?.getRuleState("speech_doll_talk")?.inEffect) {
						...
						if (maxWordLength > 0 && words.some(word => word.length > maxWordLength)) {
							return true;
						}
					}
				}
				if (Sensitivity >= 1) {
					if (BCX?.getRuleState("speech_ban_words")?.inEffect) {
						const bannedWords = [];
						for (let word of BCX?.getRuleState("speech_ban_words")?.customData.bannedWords) {
		...
		SDK.hookFunction("ServerSend", 1, (args, next) => {
			...
			if (fbcSettings.antiAntiGarbleExtra && message == "ChatRoomChat" && data.Type === "Chat") {
				const gagLevel = getTotalGagLevel(Player);
				if (gagLevel > 0) {
					const msg = data.Content;
					const soundWhitelist = [];
					if (BCX?.getRuleState("speech_specific_sound")?.inEffect) {
						for (let word of BCX?.getRuleState("speech_specific_sound")?.customData.soundWhitelist) {
							word = word.toLocaleLowerCase();
							if (!soundWhitelist.includes(word)) {
								soundWhitelist.push(word);
							}
						}
					}
					...
					while (i < msg.length) {
						...
						if (useWhitelist) {
							...
							debug(`Try to garble ${msg[i]} at ${i}, bestMatch is ${bestMatch}`);
							if (bestMatch > i) {
								allowHyphen = false;
			...
			// Original AntiAntiGarble behavior
	...
	async function antiLoosen() {
		...
		SDK.hookFunction(
			...
			(args, next) => {
				...
				let valid = true;
				if (fbcSettings.antiLoosenOwnerLock) {
			...
			(
				...
				if (data.Result && fbcSettings.instantMessenger) {
					...
					if (!data.Result.some((f) => f.MemberNumber === activeChat) ||
						(forbid && !allowedMembers.includes(activeChat))) {
